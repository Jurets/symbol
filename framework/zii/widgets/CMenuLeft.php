 ...
 * @since 1.1
 */
class CMenuLeft extends CWidget
{
	/**
...
//			echo CHtml::closeTag('ul');
	...
	protected function renderMenuRecursive($items)
	{
		...
		foreach($items as $item)
		{
			...
			$options2['class'] = 'acc_trigger';

			if (isset($item['items2']))
			{
				echo CHtml::openTag ('li', $options2)."\n";
				echo '<a>' . $item['label'] . '</a>';
			...
			else
			{
            	...
            	echo $this->renderMenuItem($item, $options);
				echo CHtml::closeTag ('li')."\n\t";
			}

			if(isset($item['items2']) && count($item['items2']))
			{
				$options['class'] = 'acc_container';
				echo CHtml::openTag('ul', $options)."\n\t";
				$this->renderMenuRecursive($item['items2']);
				echo CHtml::closeTag('ul')."\n";
			}
		}
	}
	...
	protected function normalizeItems($items,$route,&$active)
	{
		foreach($items as $i=>$item)
		{
			...
			if(isset($item['items2']))
			{
				$items[$i]['items2']=$this->normalizeItems($item['items2'],$route,$hasActiveChild);
				if(empty($items[$i]['items2']) && $this->hideEmptyItems)
				{
					unset($items[$i]['items2']);
					if(!isset($item['url']))
					{
						unset($items[$i]);
						continue;

...
class CategoryController extends SAdminController {
    ...
    public function actionUpdate($new = false)
    {
        ...
        $cr = new CDbCriteria;
        $cr->addNotInCondition('StoreAttribute.id', CHtml::listData($model->categoryAttributeRelation, 'attribute_id','attribute_id'));
        $allAttributes = StoreAttribute::model()->findAll($cr);
        $filterTab = $this->renderPartial('_options', array('model'=>$model,'attributes'=>$allAttributes), true);
        $form->content = array(Yii::t('main', 'Фильтры')=> $filterTab);
//        $form->tabs = array('filter'=> $x);

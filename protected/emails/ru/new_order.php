...
<body>
  ...
  <p>
    ...
    <p>
      <b>Всего к оплате:</b>
      <?=StoreProduct::formatPrice($order->total_price + $order->delivery_price)?> р.
    </p>

    <p>
      <b>Контактные данные:</b><br/>
      <?= implode('<br/>', array($order->user_name, $order->user_phone, $order->user_country . ', ' . $order->user_city . ', ' . $order->user_address . ', ' . $order->user_house)) ?>
    </p>
